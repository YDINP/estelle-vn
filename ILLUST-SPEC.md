...
>
> ⚠️ **외형의 정본은 이 문서가 아니라 실제 납품 아트 `public/char/<id>/soft.webp` 다.**
> 아래 서술은 그 아트에 맞춰 2026-07-27에 교정됐다(이전 서술은 전신 리드로우 이전 버전이라
> 벨포르·벨리안·리비아·레이먼의 머리색·눈색·복장이 실제와 달랐고, 그 텍스트를 앵커로 쓴
> CG가 전부 딴사람으로 생성되는 사고가 있었다).
> **CG 생성 시에는 텍스트 대신 캐릭터 아트를 레퍼런스 이미지로 넘겨라** — `tools/cg-generate.mjs` 가 그렇게 한다.

### 1.1 릴리아 (id: `lilia`) — 하이델 공작영애 · 거울/희생 · 테마 #ffdf9e
- **외형**: 20대 초, 물결치는 **밀빛금발** 롱헤어(반묶음, 꽃·리본 장식), **맑은 블루 아이**, 흰 피부, 단아·기품.
- **의상**: 세이지그린 오버드레스 + 크림 언더스커트의 **소박한 영애복**(긴소매 블라우스, 오프숄더 아님),
  금사 자수 밑단, 금색 리본 벨트에 에메랄드, 목에 에메랄드 초커. 볼가운급 볼륨이 아니라 단정한 실루엣.
  채색은 밝은 파스텔 계열. 봄·볕의 상징, 화려하되 절제.
...
- **외형**: 20대 중, 짙은 **와인빛 적발 롱 웨이브**(내린 머리, 옆머리에 보석 핀), **붉은 눈**,
  매혹적·오만한 화려함. 진홍+검정 로코코 볼가운에 금사슬 드레이프, 검은 레이스 초커.
...
- **외형**: 20대 중, 선명한 **적발**(짧고 살짝 흐트러진 우아함), **앰버/골드빛 눈**, 위험한 미소의 미남.
  검정+금 자수 대형 케이프(모피 어깨), 붉은 안감, 가슴의 붉은 선버스트 브로치, 검은 장갑·바지·부츠.
  ⚠️ 구 서술의 '금발'은 오기 — 실제 아트는 적발이다.
...
- **외형**: 20대 후, 단정한 **갈색 숏컷**, **황금빛 눈**, 평민 출신의 강직한 미남.
  **감청색(네이비) 군복 코트**(무릎길이) + 은색 견장·체인 장식, 검은 장갑, 검은 롱부츠, 검을 패용.
  ⚠️ 구 서술의 '백금발·회청색 눈·판금 갑옷'은 전부 오기 — 실제 아트와 다르다.
...
- **외형**: 30대 초, **흑발 숏컷**, **서늘한 파란 눈**, 과묵한 북방 무인, 굳은 이목구비.
  전신 검정 — 흑색 깃털 장식 대형 망토, 흑금 군복, 은사슬·별 브로치, 검은 장갑·롱부츠, 장검 패용.
  ⚠️ 구 서술의 '블루그레이 헤어·회색 눈'은 오기.
...
- **외형**: 20대 중, **은백~라벤더 롱 스트레이트**(앞머리 있음), **보라 눈**, 얼음처럼 완전한 미모, 흠 없는 자세.
  **연보라/라일락 시폰 드레스**(오프숄더, 층층 시스루 오버레이, 은금 자수), 리본 벨트.
  ⚠️ 구 서술의 '아이스블루 눈·흰파랑 드레스'는 실제와 색조가 다르다.
...
- **외형**: 20대 초, **갈색 롱 웨이브**(옆머리에 금색 별 장식), **황금빛 눈**, 조심스러운 눈빛.
  **짙은 녹색 오프숄더 귀족 드레스**(금사 자수, 크림 언더스커트, 퍼프 소매). 서녀이지만 차림은 귀족.
  ⚠️ 구 서술의 '10대 시골 소녀·수수한 베이지 옷'은 오기 — 실제 아트는 성인 귀족 여성이다.
...
- **비고**: 실루엣 1종만. 메피안과 달리 **원화는 아직 없다**(§1.9의 CSS 필터 방식 전환 가능).

---

## 1.11 조연 레퍼런스 시트 (2026-07-29 신설)

이 인물들은 `characters.ts` 에 **없다** — `Speaker` 타입상 발화할 수 없고 **CG에만 등장**한다.
그런데 레퍼런스 아트가 없어 CG마다 새로 그려지고, 같은 인물이 컷마다 딴사람으로 나왔다.

- 시트 위치: **`art/refs/<id>.webp`** (배경 투명, 전신 중립 포즈)
  → `public/` 밖이라 **배포에 포함되지 않는다.** `tools/cg-generate.mjs` 가 정체성 레퍼런스로만 쓴다.
- 생성: `node tools/gen-support-chars.mjs` → `tools/cutout-bg.mjs` 로 배경 투명화
- 외형·성격은 전부 **프로즈 근거**에 맞췄다(`gen-support-chars.mjs` 의 `근거` 주석 참조)

| id | 인물 | CG 등장 | 프로즈 근거로 고정한 핵심 |
|---|---|---|---|
| `butler` | 하이델가 집사 | **6컷** | 위증 공모자·확보 표적. 완벽한 공손함 뒤의 계산 |
| `bishop` | 주교 | 2컷 | 대관식에서 왕관을 들어 올리는 성직자의 손 |
| `chamberlain` | 궁 시종장 | 1컷 | **은쟁반을 든 손이 떨린다**(7·21·22화 복선 → 28화 독배 바꿔치기) |
| `ornel` | 오르넬 | 1컷 | **노인**이다("목소리는 늙었고"). 성물매매로 파문 → **문장이 뜯긴 사제복**. 죽은 것으로 기록됨 |
| `old_retainer` | 북부 노신하 | 1컷 | **오른손이 잘려 붕대도 없이 드러남**, 왼손을 들어 보인다 |
| `kyle` | 서기관 카일 | 1컷 | 재상부 서기관·이중스파이. 잉크 묻은 손, 곁눈질 |
| `duke` | 하이델 공작 | **0컷** | 릴리아의 아버지. 하이델 가문색(세이지그린+금)·**서명한 손의 붕대**. 향후 컷 대비 선제작 |

⚠️ **`공작` 문자열은 `공작영애`(릴리아)·`공작저`·`공작가`와 겹친다.** 부정 전방탐색
`/공작(?!영애|저|가)/` 없이 세면 릴리아 등장 컷이 공작으로 오집계된다(실제로 7컷 오탐이 있었다).
