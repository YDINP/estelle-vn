...
export interface Character {
  ...
  bustZoom?: number;         // 반신(머리~허리) CSS 확대율. 기본 1(=전신 그대로). marion만 전신이라 ≈1.44
...
export const CHARACTERS: Record<CharacterId, Character> = {
  ...
  // 가시 돋친 장미 영애 (적발·와인 드레스). ⚠️ body가 유일하게 전신(발끝)이라 반신은 bustZoom으로 크롭.
  marion: {
    id: "marion", name: "마리온", hasPortrait: true, color: "#ff96a6", // 적발 — 로즈레드
    body: GRID_EMOTIONS,
    bustZoom: 1.44, // 928/645 — 전신 body를 머리~허리(구 bust_ 크롭)로 확대. 기준 캐릭터
    fallback: {},
  },
  ...
  lucienne: {
    ...
    body: GRID_EMOTIONS, fallback: {}, // 후작영애. 얼음처럼 완전한, 금 간 완벽주의
  },
  // id 'livia'는 아트 폴더/세이브 호환용 — 표시명 '리비아'(공작가의 서녀)
  livia: {
    id: "livia", name: "리비아", hasPortrait: true, color: "#f2cfa6", // 갈색머리 — 웜 베이지
    body: GRID_EMOTIONS, fallback: {}, // 하이델 공작가의 서녀. 그늘에서 숨죽여 피는 들꽃
  },
  reimon: {
    id: "reimon", name: "레이먼", hasPortrait: true, color: "#b9c9dd", // 스틸 블루그레이
    body: GRID_EMOTIONS, fallback: {}, // 근위대 기사단장이자 북부대공. 검보다 차가운 침묵
  },
  azael: {
    id: "azael", name: "아젤", hasPortrait: true, color: "#ece5cf", // 아이보리 실버
    body: GRID_EMOTIONS, fallback: {}, // 신성왕국의 성기사. 빛의 맹세를 검에 새긴 자
  },
  // ── 캐릭터 추가 템플릿 ──
...
/** 반신(머리~허리) CSS 확대율 — body가 전신형인 캐릭터만 >1 */
