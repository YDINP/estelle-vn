...
### CG 현황 — 3단계가 섞여 있다 (2026-07-28)

| 단계 | 수 | 상태 |
|---|---|---|
| **v3** | 49 | 여성 4인(릴리아·마리온·루시엔·리비아) 등장 컷. **완성본** — 장면에 맞는 포즈·표정, 캐릭터별 성격 반영, 배경과 조명·화풍 통합 |
| **v2** | 32 | 남성 위주. 캐릭터 외형은 맞지만 **레퍼런스 포즈가 그대로 딸려 와 붙여 넣은 느낌**. v3 절차로 재생성 필요 |
| 미배치 | 15 | 벨포르·벨리안·레이먼의 후반부/엔딩. 플레이스홀더 표시 중 |

**v2 → v3 재생성 방법** (여성분과 동일):
```bash
node tools/cg-worklist.mjs --emit --all
# 해당 컷의 배포본·스테이징 PNG를 art-backup/ 으로 옮긴 뒤
node tools/cg-generate.mjs --route belfor --batch 2      # belian / reimon / azael 도 동일
node tools/cg-place.mjs --force
```
`cg-generate.mjs`가 캐릭터 아트를 정체성 레퍼런스로 넘기고, 포즈·표정·조명은 장면이 정하며,
캐릭터별 연기 지시(성격)를 함께 실어 보낸다. 남성 4인 연기 지시도 이미 들어 있다.

⚠️ `tools/cg-regen-report.mjs` 는 스테이징 PNG 유무로만 판정해 **v2와 v3를 구분하지 못한다** —
"레퍼런스 반영"으로 집계되는 것 중 남성분은 아직 v2다.

### 🔴 (이전 기록) CG 구버전 캐릭터 외형 문제
